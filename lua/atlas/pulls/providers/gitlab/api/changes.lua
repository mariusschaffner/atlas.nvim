...
---@param opts { force_refresh: boolean|nil }|nil
---@param on_done fun(additions: integer|nil, deletions: integer|nil, err: string|nil)
---@return { cancel: fun() }|nil
function M.fetch_commit_stats(pr, commit, opts, on_done)
	opts = opts or {}
	local path = pr.repo_full_name
	local hash = tostring(commit.hash or "")
	if path == "" or hash == "" then
		vim.schedule(function()
			on_done(nil, nil, "Invalid commit identifier")
		end)
		return nil
	end

	local cache_key = string.format("gitlab_pulls:commit_stats:%s:%s", path, hash)
	if not opts.force_refresh then
		local cached, ok = service.get_memory_cache(cache_key)
		if ok then
			on_done(cached.additions, cached.deletions, nil)
			return nil
		end
	end

	local endpoint = string.format("/projects/%s/repository/commits/%s", service.url_encode(path), hash)
	return service.request("GET", endpoint, nil, function(result, err)
		if err then
			on_done(nil, nil, err)
			return
		end
		local raw = json.safe_table(result)
		local stats = json.safe_table(raw.stats)
		local additions = tonumber(stats.additions)
		local deletions = tonumber(stats.deletions)
		service.set_memory_cache(cache_key, { additions = additions, deletions = deletions })
		on_done(additions, deletions, nil)
	end, {
		action = "Fetch commit stats",
		project_path = path,
		hash = hash,
	})
end
