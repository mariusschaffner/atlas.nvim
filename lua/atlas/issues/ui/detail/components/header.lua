...
-- No second column to show: keep the original single-column layout exactly
-- as before, rather than reserving empty space for a k2/v2 pair that would
-- never have content.
---@param primary IssuesDetailHeaderField[]
---@param width integer
---@return string[], table[]
local function render_single_column(primary, width)
	local rows = {}
...
---@param primary IssuesDetailHeaderField[]
---@param secondary IssuesDetailHeaderField[]
---@param width integer
---@return string[], table[]
local function render_two_column(primary, secondary, width)
...
function M.render(issue, width, fields, secondary_fields)
	...
	if #secondary == 0 then
		table_lines, table_spans = render_single_column(primary, width)
	else
		table_lines, table_spans = render_two_column(primary, secondary, width)
	end
